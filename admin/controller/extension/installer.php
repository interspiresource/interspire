...
class ControllerExtensionInstaller extends Controller {
		...
		if (!$json) {
			...
			if ($xml) {
                ...
                try {
                    ...
                    #Xml replace text '', Because It doesn't necessary Interspire
                    $xml = '';
	...
	public function install() {
		$this->load->language('extension/installer');
		$json = array();
		$data = $this->utility->getRemoteData(html_entity_decode("http://Interspire.io/" . rtrim($this->request->post['store'], 's') . "/1.0/download/" . $this->request->post['product_id'] . "/latest/" . VERSION . "/" . $this->config->get('api_key')), array('referrer' => true));
